...
def parse_picklist(text):
    ...
    card_pattern = re.compile(r"Select Your Card:\s*([\d/]+)\s+([^(]+)\(([^)]+)\)", re.IGNORECASE)
    quantity_pattern = re.compile(r"Quantity[:\s]+(\d+)", re.IGNORECASE)
    ...
    # Flatten into dataframe and per-buyer summary
    summary_dict = {}
